...
def prepare():
    ...
    md5data = md5(open('archive/data').read()).hexdigest()
    ...
    try:
        import lzma
        doXZ = subprocess.call("xz --check=crc32 --lzma2=dict=1MiB < archive.cpio > archive.cpio.xz", shell=True) == 0
    except:
        doXZ = False
    ...
    def archiveExtract(self, fn, fmt='r|*'):
        ...
        for f in arc:
            if f.isfile():
                ...
                self.assertEqual(md5data, md5(data).hexdigest())
                found = True
